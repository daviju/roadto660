...
export function Charts() {
  ...
  const pieData = useMemo(() => Object.entries(byCategory).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value), [byCategory]);
  ...
  return (
    <motion.div className="space-y-6" variants={staggerContainer} initial="initial" animate="animate">
      ...
      <motion.div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6" variants={staggerContainer}>
        <motion.div variants={fadeUp} className="bg-th-card rounded-xl p-4 md:p-5 border border-th-border card-glow">
          <h3 className="text-sm font-semibold text-th-text mb-4">Gastos por categoria</h3>
          {barData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
        ...
        <motion.div variants={fadeUp} className="bg-th-card rounded-xl p-4 md:p-5 border border-th-border card-glow">
          <h3 className="text-sm font-semibold text-th-text mb-4">Distribucion</h3>
          {pieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
