...
import { Check, Circle, Clock, ChevronDown, Pencil, Trash2, Plus, X, Flag } from 'lucide-react';
...
interface EditItemState { itemId: string; name: string; cost: string; }
interface EditPhaseState { phaseId: string; name: string; targetDate: string; }

export function Timeline() {
  const {
    phases, togglePhaseItem, updatePhaseStatus, updatePhaseItem,
  ...
  const saveEditItem = (phaseId: string) => {
    if (!editingItem) return;
    const cost = parseFloat(editingItem.cost);
    if (editingItem.name.trim() && !isNaN(cost) && cost > 0) {
      updatePhaseItem(phaseId, editingItem.itemId, { name: editingItem.name.trim(), estimatedCost: cost });
    }
    setEditingItem(null);
  };
  ...
  const saveEditPhase = () => {
    ...
    updatePhase(editingPhase.phaseId, {
      name: editingPhase.name.trim(),
      targetDate: editingPhase.targetDate || undefined,
    });
  ...
  const handleAddPhase = async () => {
    ...
    await addPhase(newPhaseName.trim(), newPhaseDate || undefined);
    setNewPhaseName(''); setNewPhaseDate(''); setAddingPhase(false);
  };

  const handleDeletePhase = (phaseId: string, phaseName: string) => {
    if (confirm(`Eliminar la fase "${phaseName}" y todos sus items?`)) {
      removePhase(phaseId);
    }
  };
    ...
    <motion.div className="space-y-6" variants={staggerContainer} initial="initial" animate="animate">
      ...
      <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-th-muted mt-1">Hoja de ruta hacia tu objetivo</p>
        </div>
        <motion.button
          onClick={() => { setAddingPhase(true); }}
          className="flex items-center gap-2 px-4 py-2 bg-accent-purple text-white rounded-lg text-sm font-medium hover:bg-accent-purple/80 transition-colors self-start"
      ...
      <AnimatePresence>
        {addingPhase && (
          <motion.div
            className="bg-th-card rounded-xl p-4 border border-accent-purple/30 space-y-3"
            initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
          >
            <h3 className="text-sm font-semibold text-th-text">Nueva fase</h3>
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                type="text" value={newPhaseName} onChange={(e) => setNewPhaseName(e.target.value)}
                placeholder="Nombre de la fase"
                className="flex-1 bg-th-input border border-th-border-strong rounded-lg px-3 py-2 text-sm text-th-text focus:border-accent-purple focus:outline-none"
                onKeyDown={(e) => { if (e.key === 'Enter') handleAddPhase(); if (e.key === 'Escape') { setAddingPhase(false); setNewPhaseName(''); setNewPhaseDate(''); } }}
                autoFocus
              />
              <input
                type="date" value={newPhaseDate} onChange={(e) => setNewPhaseDate(e.target.value)}
                className="w-full sm:w-44 bg-th-input border border-th-border-strong rounded-lg px-3 py-2 text-sm text-th-text focus:border-accent-purple focus:outline-none"
              />
            </div>
            <div className="flex gap-2">
              <motion.button onClick={handleAddPhase}
                className="px-4 py-1.5 bg-accent-purple text-white rounded-lg text-sm font-medium hover:bg-accent-purple/80 transition-colors"
                whileTap={{ scale: 0.95 }}>
                Crear fase
              </motion.button>
              <motion.button onClick={() => { setAddingPhase(false); setNewPhaseName(''); setNewPhaseDate(''); }}
                className="px-4 py-1.5 bg-th-hover text-th-secondary rounded-lg text-sm font-medium hover:text-th-text transition-colors"
                whileTap={{ scale: 0.95 }}>
                Cancelar
              </motion.button>
            </div>
      ...
      {phases.length === 0 && !addingPhase && (
        <motion.div
          variants={fadeUp}
          className="bg-th-card rounded-xl p-10 border border-th-border text-center"
        >
          <motion.div
          ...
          >
            <Flag size={28} className="text-accent-purple" />
          </motion.div>
          <h3 className="text-lg font-semibold text-th-text mb-2">No hay fases en tu hoja de ruta</h3>
          <p className="text-sm text-th-muted mb-5 max-w-sm mx-auto">
            Crea fases para organizar los pasos hacia tu objetivo. Cada fase puede contener items con sus costes.
          </p>
          <motion.button
            onClick={() => setAddingPhase(true)}
            className="px-5 py-2.5 bg-accent-purple text-white rounded-xl text-sm font-medium hover:bg-accent-purple/80 transition-colors"
                    ...
                    {isEditingThisPhase ? (
                      <div className="p-4 space-y-2">
                        <div className="flex flex-col sm:flex-row gap-2">
                          <input
                            type="text" value={editingPhase.name}
                            onChange={(e) => setEditingPhase({ ...editingPhase, name: e.target.value })}
                            className="flex-1 bg-th-input border border-th-border-strong rounded-lg px-3 py-1.5 text-sm text-th-text focus:border-accent-purple focus:outline-none"
                            onKeyDown={(e) => { if (e.key === 'Enter') saveEditPhase(); if (e.key === 'Escape') setEditingPhase(null); }}
                            autoFocus
                          />
                          <input
                            type="date" value={editingPhase.targetDate}
                            onChange={(e) => setEditingPhase({ ...editingPhase, targetDate: e.target.value })}
                            className="w-full sm:w-44 bg-th-input border border-th-border-strong rounded-lg px-3 py-1.5 text-sm text-th-text focus:border-accent-purple focus:outline-none"
                          />
                        </div>
                        <div className="flex gap-1">
                          <motion.button onClick={saveEditPhase} className="p-1.5 text-accent-green hover:bg-accent-green/10 rounded" whileTap={{ scale: 0.85 }}><Check size={14} /></motion.button>
                          <motion.button onClick={() => setEditingPhase(null)} className="p-1.5 text-th-secondary hover:bg-th-hover rounded" whileTap={{ scale: 0.85 }}><X size={14} /></motion.button>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-stretch">
                        <motion.button
                          ...
                          className="flex-1 p-4 md:p-5 flex items-center justify-between hover:bg-th-hover-subtle transition-colors text-left min-w-0"
                        ...
                        >
                          <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] md:text-xs text-th-muted mt-0.5">
                                {phase.targetDate ? `${daysLeft}d restantes · ${formatDate(phase.targetDate)}` : 'Sin fecha'}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex flex-col border-l border-th-border">
                          <motion.button
                            onClick={() => setEditingPhase({ phaseId: phase.id, name: phase.name, targetDate: phase.targetDate || '' })}
                            className="flex-1 px-3 text-th-faint hover:text-th-text hover:bg-th-hover transition-colors"
                            whileTap={{ scale: 0.85 }} aria-label={`Editar fase ${phase.name}`}
                          >
                            <Pencil size={13} />
                          </motion.button>
                          <motion.button
                            onClick={() => handleDeletePhase(phase.id, phase.name)}
                            className="flex-1 px-3 text-th-faint hover:text-accent-red hover:bg-accent-red/10 transition-colors border-t border-th-border"
                            whileTap={{ scale: 0.85 }} aria-label={`Eliminar fase ${phase.name}`}
                          >
                            <Trash2 size={13} />
                          </motion.button>
                        </div>
                                  ...
                                  {isEditingItem ? (
                                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full">
                                      <input type="text" value={editingItem.name} onChange={(e) => setEditingItem({ ...editingItem, name: e.target.value })}
                                        ...
                                        onKeyDown={(e) => { if (e.key === 'Enter') saveEditItem(phase.id); if (e.key === 'Escape') setEditingItem(null); }} autoFocus />
                                      <input type="number" step="0.01" value={editingItem.cost} onChange={(e) => setEditingItem({ ...editingItem, cost: e.target.value })}
                                        ...
                                        onKeyDown={(e) => { if (e.key === 'Enter') saveEditItem(phase.id); if (e.key === 'Escape') setEditingItem(null); }} />
                                      <div className="flex gap-1 justify-end">
                                        <motion.button onClick={() => saveEditItem(phase.id)} className="p-1.5 text-accent-green hover:bg-accent-green/10 rounded" whileTap={{ scale: 0.85 }} aria-label="Guardar"><Check size={14} /></motion.button>
                                        <motion.button onClick={() => setEditingItem(null)} className="p-1.5 text-th-secondary hover:bg-th-hover rounded" whileTap={{ scale: 0.85 }} aria-label="Cancelar"><X size={14} /></motion.button>
                                      </div>
                                    </div>
                                  ) : (
                                    <>
                                      ...
                                      <div className="flex items-center gap-1.5 ml-8 sm:ml-0">
                                        ...
                                        <motion.button onClick={() => setEditingItem({ itemId: item.id, name: item.name, cost: item.estimatedCost.toString() })}
                          ...
                          <AnimatePresence>
                            {addingTo === phase.id ? (
                              <motion.div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-3 rounded-lg border border-dashed border-th-border-strong bg-th-hover-subtle"
                                initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                                <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nombre del item"
                                  className="flex-1 bg-th-input border border-th-border-strong rounded px-3 py-1.5 text-sm text-th-text focus:outline-none"
