...
export function Dashboard() {
  ...
  return (
    ...
    >
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Tu progreso hacia la {activeMoto?.name || 'moto'}
          </p>
        </div>
        {activeMoto && (
          <motion.button
          ...
          >
            <Bike size={16} />
            {activeMoto.name} - {formatCurrency(activeMoto.price)}
            <ArrowRight size={14} />
          </motion.button>
        )}
      </motion.div>
      ...
      <motion.div
        variants={fadeUp}
        className="bg-surface rounded-xl p-6 border border-white/5 card-glow"
        ref={progressRef}
      >
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-white">Progreso global</span>
          <motion.span
        ...
        <div className="w-full h-4 bg-white/5 rounded-full overflow-hidden mb-3">
          <motion.div
            className="h-full rounded-full bg-gradient-to-r from-accent-purple to-purple-400 progress-shimmer"
            initial={{ width: 0 }}
      ...
      <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" variants={staggerContainer}>
        <motion.div variants={fadeUp}>
          <KPICard title="Saldo disponible" value={formatCurrency(available)}
            subtitle={`Saldo total: ${formatCurrency(settings.currentBalance)}`}
            icon={<Wallet size={18} />} color="text-accent-green" glowClass="card-glow-green" />
        </motion.div>
        <motion.div variants={fadeUp}>
          <KPICard title="Ahorro este mes"
        ...
        <motion.div variants={fadeUp}>
          <KPICard title="Dias restantes" value={`${daysLeft}`}
            subtitle={`Objetivo: ${new Date(settings.targetDate).toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}`}
        ...
        <motion.div variants={fadeUp}>
          <KPICard title="Ritmo necesario" value={formatCurrency(requiredMonthly)}
            subtitle="ahorro/mes para llegar"
      ...
      <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-4" variants={staggerContainer}>
        <motion.div variants={fadeUp} className="bg-surface rounded-xl p-5 border border-white/5 card-glow" whileHover={{ y: -2 }}>
          <h4 className="text-xs text-gray-500 uppercase tracking-wider mb-3">Resumen mensual</h4>
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono text-accent-red">{formatCurrency(29.73)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Gastos variables</span>
              <span className="font-mono text-accent-red">{formatCurrency(monthExpenses)}</span>
            </div>
            <div className="border-t border-white/5 pt-2 flex justify-between text-sm font-medium">
              <span className="text-gray-300">Ahorro estimado</span>
              <span className="font-mono text-accent-green">{formatCurrency(totalMonthlyIncome - 29.73 - monthExpenses)}</span>
            </div>
          </div>
        </motion.div>
        <motion.div variants={fadeUp} className="bg-surface rounded-xl p-5 border border-white/5 card-glow" whileHover={{ y: -2 }}>
          <h4 className="text-xs text-gray-500 uppercase tracking-wider mb-3">Falta por ahorrar</h4>
          <p className="font-mono text-3xl font-bold text-white">{formatCurrency(Math.max(0, totalObjective - progress))}</p>
          <p className="text-xs text-gray-500 mt-2">de {formatCurrency(totalObjective)} total</p>
        </motion.div>
        <motion.div variants={fadeUp} className="bg-surface rounded-xl p-5 border border-white/5 card-glow" whileHover={{ y: -2 }}>
          <h4 className="text-xs text-gray-500 uppercase tracking-wider mb-3">Items pagados</h4>
          <p className="font-mono text-3xl font-bold text-white">
            {phases.reduce((s, p) => s + p.items.filter((i) => i.paid).length, 0)}
            <span className="text-lg text-gray-500">/{phases.reduce((s, p) => s + p.items.length, 0)}</span>
          </p>
          <p className="text-xs text-gray-500 mt-2">Pagado: {formatCurrency(totalPaid)}</p>
        </motion.div>
      </motion.div>
    </motion.div>
  );
